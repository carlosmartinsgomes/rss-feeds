...
def monthly_sampling(timestamps, max_candidates_per_month=3):
    ...
    for ym, tss in by_month.items():
        ...
        # tentar 3 recentes + 3 antigos
        candidates = tss_sorted[:3] + tss_sorted[-3:]
...
def get_closest_snapshot(url, year, month):
    # usar dia 15 como referência neutra
    ts = f"{year}{month:02d}15"
    ...
    try:
        ...
        if "archived_snapshots" in j and "closest" in j["archived_snapshots"]:
            return j["archived_snapshots"]["closest"].get("timestamp")
    except:
        return None
    return None
