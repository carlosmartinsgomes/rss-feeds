...
# (o teu scraper que já funcionava)
# ---------------------------
...
# ---------------------------
# SCRAPER específico MODERNHEALTHCARE
# (novo: segue os selectors / outerHTML que nos deste)
# ---------------------------
def scrape_modernhealth_listing(base_url="https://www.modernhealthcare.com/latest-news/", max_items=10, timeout=12):
    try:
        r = requests.get(base_url, headers={'User-Agent': 'Mozilla/5.0'}, timeout=timeout)
        r.raise_for_status()
        html = r.text
    except Exception as e:
        print("scrape_modernhealth_listing: fetch failed:", e)
        return []

    soup = BeautifulSoup(html, "html.parser")

    # elimina modais / subscribe se constarem no HTML estático
    for sel in ['div[class*="subscribe"]', '.newsletter-popup', '.modal', '[role="dialog"]', '.subscribe-panel']:
    ...
    def find_wrapper_for_title(el):
        cur = el
        for _ in range(10):
            if cur is None:
                break
            # prefer article or div with u-border-b (structure you pointed out)
            classes = ' '.join(cur.get('class') or [])
            if cur.name == 'article' or 'u-border-b' in classes or 'col-lg-6' in classes or 'views-row' in classes:
                return cur
            cur = cur.parent
        # fallback: closest article or parent
        a = el.find_parent('article')
        if a:
            return a
        return el.parent or el

    def find_title(wrapper, known_title_el=None):
        if known_title_el and text_of(known_title_el):
            return text_of(known_title_el)
        for sel in ['span.u-text-text-dark', "a[aria-label^='Title'] span", '.content-list-title a', 'h2', 'h3', 'h4']:
    ...
    def find_link(wrapper, known_title_el=None):
        # prefer anchor around known title
        if known_title_el:
            a = known_title_el.find_parent('a', href=True)
            if a and a.get('href'):
                h = a.get('href').strip()
                if h and not re.search(r'(^#|/subscribe|mailto:|javascript:)', h, re.I):
                    return abs_url(h, base_url)
        # common patterns
        for sel in ('a[aria-label^="Title"][href]', 'a.content-list-title[href]', '.content-list-title a[href]', 'a[href]'):
        ...
        if a and a.get('href'):
            h = a.get('href').strip()
            if not re.search(r'(^#|/subscribe|mailto:|javascript:)', h, re.I):
                return abs_url(h, base_url)
        return ''

    def find_date(wrapper):
        # check group-author-line first (top variant)
        anc = wrapper.find_parent('article') or wrapper
        group = anc.select_one('div.group-author-line, div.field.field--name-field-author')
        if group:
            spans = [text_of(s) for s in group.select('span') if text_of(s)]
            if len(spans) >= 2:
    ...
    def find_description(wrapper):
        for sel in ('div.u-h-auto.u-w-full.u-font-secondary p', 'div.field.field--name-field-subheader.field--item', '.dek', '.summary', 'p'):
            el = wrapper.select_one(sel)
            if el:
                t = text_of(el)
                if t and not re.search(r'subscribe|image', t, re.I):
    ...
    # collect title elements in the top block first
    title_els = main.select('span.u-text-text-dark, a[aria-label^="Title"] span')
    items = []
    seen = set()
    ...
    for el in title_els:
        ...
        if is_bad_title(ttext):
            continue
        wrapper = find_wrapper_for_title(el)
        if not wrapper:
            continue
    ...
    # if not enough, scan article/list blocks in order
    if len(items) < max_items:
        candidates = main.select('article, .u-border-b, .views-row, .view-content > div, .col-lg-6') or []
        for wrapper in candidates:
            if len(items) >= max_items:
                break
            tsel = wrapper.select_one('span.u-text-text-dark, a[aria-label^="Title"] span, .content-list-title a, a.title, h2, h3')
            ttext = text_of(tsel) if tsel else ''
            if is_bad_title(ttext):
                continue
    ...
    if len(items) < max_items:
        for a in main.select('a[href]'):
            ...
            if not h or re.search(r'(^#|/subscribe|mailto:|javascript:)', h, re.I):
                continue
            full = abs_url(h, base_url)
            if full in seen:
                continue
            t = text_of(a)
            if not t or len(t) < 6:
...
def main():
    ...
    for ff in feed_files:
        try:
            ...
            # special: modernhealthcare
            if site_name == "modernhealthcare":
                modern_items = scrape_modernhealth_listing(base_url="https://www.modernhealthcare.com/latest-news/", max_items=10, timeout=12)
                for it in modern_items:
                    rows = {
                        "site": site_name,
                        "title": it.get("title", "") or "",
                        "link (source)": it.get("link", "") or "",
