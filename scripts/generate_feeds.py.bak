#!/usr/bin/env python3
# scripts/generate_feeds.py
import os
import json
import re
import sys
from bs4 import BeautifulSoup
import requests
from feedgen.feed import FeedGenerator
from datetime import datetime
from urllib.parse import urljoin, urlparse, urlunparse, parse_qsl, urlencode

# small compatibility shim to silence UnknownTimezoneWarning for "ET"
import warnings
from dateutil import parser as dateparser
from dateutil import tz as date_tz

# ---- ignore some dateutil warnings (best-effort) ----
try:
    from dateutil import _parser as _dateutil__parser
    UnknownTimezoneWarning = _dateutil__parser.UnknownTimezoneWarning
    warnings.filterwarnings("ignore", category=UnknownTimezoneWarning)
except Exception:
    warnings.filterwarnings("ignore", message="tzname .* identified but not understood")

# map "ET" -> America/New_York by default when parse is called without tzinfos
_default_tzinfos = {"ET": date_tz.gettz("America/New_York")}
_original_parse = dateparser.parse


def _parse_with_default_tzinfos(timestr, *args, **kwargs):
    if "tzinfos" not in kwargs or kwargs["tzinfos"] is None:
        kwargs["tzinfos"] = _default_tzinfos
    return _original_parse(timestr, *args, **kwargs)


dateparser.parse = _parse_with_default_tzinfos

# ---- filters for bad hrefs/titles (skip cookie/help/legal/etc) ----
bad_href_re = re.compile(r'(^#|/help|/legal|cookie|privacy|terms|signin|login|settings|/consent|/preferences|/policies|mailto:)', re.I)
bad_title_re = re.compile(r'^(send feedback|cookie policy|copyright policy|community guidelines|user agreement|return to homepage|home|legal|privacy policy)$', re.I)

ROOT = os.path.dirname(__file__)
SITES_JSON = os.path.join(ROOT, 'sites.json')


def load_sites():
    j = json.load(open(SITES_JSON, 'r', encoding='utf-8'))
    return j.get('sites', [])


def fetch_html(url, timeout=20):
    headers = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/120.0 Safari/537.36',
        'Accept-Language': 'en-US,en;q=0.9'
    }
    r = requests.get(url, headers=headers, timeout=timeout)
    r.raise_for_status()
    return r.text


def text_of_node(node):
    if node is None:
        return ''
    return ' '.join(node.stripped_strings)


def normalize_link_for_dedupe(link):
    if not link:
        return ''
    try:
        # make absolute-ish and remove common tracking params and fragment
        p = urlparse(link)
        qs = dict(parse_qsl(p.query, keep_blank_values=True))
        for k in list(qs.keys()):
            if k.lower().startswith('utm_') or k.lower() in ('fbclid', 'gclid', 'yclid', 'mc_cid', 'mc_eid'):
                qs.pop(k)
        new_q = urlencode(qs, doseq=True)
        p2 = p._replace(query=new_q, fragment='')
        return urlunparse(p2).strip().lower()
    except Exception:
        return (link or '').strip().lower()


def extract_items_from_html(html, cfg):
    soup = BeautifulSoup(html, 'html.parser')
    container_sel = cfg.get('item_container') or 'article'
    nodes = []
    # split container selectors by comma; ignore empty
    for sel in [s.strip() for s in container_sel.split(',') if s.strip()]:
        try:
            nodes.extend(soup.select(sel))
        except Exception:
            # invalid selector -> skip
            continue

    items = []
    for node in nodes:
        title = ''
        link = ''
        date = ''
        desc = ''
        title_sel = cfg.get('title')
        link_sel = cfg.get('link')
        desc_sel = cfg.get('description')

        # Title extraction
        if title_sel:
            for s in [t.strip() for t in title_sel.split(',') if t.strip()]:
                try:
                    el = node.select_one(s)
                except Exception:
                    el = None
                if el:
                    # prefer aria-label if present (some sites put real title there)
                    if el.has_attr('aria-label') and el.get('aria-label').strip():
                        title = el.get('aria-label').strip()
                    else:
                        title = el.get_text(" ", strip=True)
                    break
        else:
            t = node.find(['h1', 'h2', 'h3', 'a'])
            if t:
                title = t.get_text(" ", strip=True)

        # Link extraction
        if link_sel:
            parts = [p.strip() for p in link_sel.split(',') if p.strip()]
            for ps in parts:
                if '@' in ps:
                    sel, attr = ps.split('@', 1)
                    try:
                        el = node.select_one(sel.strip())
                    except Exception:
                        el = None
                    if el and el.has_attr(attr):
                        raw = el.get(attr)
                        if raw:
                            link = urljoin(cfg.get('url', ''), raw)
                            break
                else:
                    try:
                        el = node.select_one(ps)
                    except Exception:
                        el = None
                    if el:
                        raw = el.get('href') if el.has_attr('href') else None
                        if raw:
                            link = urljoin(cfg.get('url', ''), raw)
                            break
        else:
            a = node.find('a')
            if a and a.has_attr('href'):
                link = urljoin(cfg.get('url', ''), a.get('href'))

        # Description extraction
        if desc_sel:
            for s in [t.strip() for t in desc_sel.split(',') if t.strip()]:
                try:
                    el = node.select_one(s)
                except Exception:
                    el = None
                if el:
                    desc = el.get_text(" ", strip=True)
                    break
        else:
            p = node.find('p')
            if p:
                desc = p.get_text(" ", strip=True)

        # Date extraction (best-effort)
        for dsel in [cfg.get('date', ''), 'time', '.date', 'span.date']:
            if not dsel:
                continue
            try:
                el = node.select_one(dsel)
                if el:
                    date = el.get_text(" ", strip=True)
                    break
            except Exception:
                continue

        # full_text fallback
        full_text = ((title or '') + ' ' + (desc or '') + ' ' + text_of_node(node)).strip()

        # apply bad href/title filters (skip items that look like navigational/legal links)
        if bad_href_re.search(link or '') or bad_title_re.search((title or '').strip()):
            # skip this node entirely
            # print debug line commented out by default; uncomment for heavy debugging
            # print(f"Skipping bad item: link='{link[:200]}' title='{(title or '')[:120]}'")
            continue

        items.append({
            'title': title,
            'link': link,
            'description': desc,
            'date': date,
            'full_text': full_text
        })
    return items


def parse_feed(items):
    # items expected as dicts with keys title, link, description, date, full_text
    return items


def matches_filters_debug(item, cfg):
    kw = cfg.get('filters', {}).get('keywords', []) or []
    exclude = cfg.get('filters', {}).get('exclude', []) or []
    if not kw and not exclude:
        return True, None
    text_title = (item.get('title', '') or '').lower()
    text_desc = (item.get('description', '') or '').lower()
    text_full = (item.get('full_text', '') or '').lower()
    text_link = (item.get('link', '') or '').lower()

    # include keywords: any match in title/desc/full/link
    if kw:
        for k in kw:
            kl = k.lower()
            if kl in text_title:
                return True, f"keyword '{k}' in title"
            if kl in text_desc:
                return True, f"keyword '{k}' in description"
            if kl in text_full:
                return True, f"keyword '{k}' in full_text"
            if kl in text_link:
                return True, f"keyword '{k}' in link"
        return False, None

    # exclude rules
    for ex in exclude:
        exl = ex.lower()
        if exl in text_title or exl in text_desc or exl in text_full:
            return False, f"exclude '{ex}' matched"
    return True, None


# ---- helper: dedupe lista de items (mantem a primeira aparição) ----
def dedupe_items(items):
    unique = {}
    out = []
    for it in (items or []):
        key = normalize_link_for_dedupe(it.get('link') or '')
        if not key:
            key = (it.get('title', '') or '').strip().lower()[:200]
        if not key:
            key = f"__no_key__{len(out)}"
        if key not in unique:
            unique[key] = True
            out.append(it)
    return out
# ---- fim helper ----


def build_feed(name, cfg, items):
    fg = FeedGenerator()
    fg.title(name)
    fg.link(href=cfg.get('url', ''), rel='alternate')
    fg.description(f'Feed gerado para {name}')
    count = 0
    for it in items:
        fe = fg.add_entry()
        fe.title(it.get('title') or 'No title')
        if it.get('link'):
            try:
                fe.link(href=it.get('link'))
            except Exception:
                # fallback: ignore broken link
                pass
        fe.description(it.get('description') or it.get('full_text') or '')
        # pubDate: try to parse to datetime
        if it.get('date'):
            try:
                parsed = dateparser.parse(it.get('date'))
                if isinstance(parsed, datetime):
                    fe.pubDate(parsed)
            except Exception:
                # ignore if parse fails
                pass
        count += 1

    outdir = os.path.join(ROOT, '..', 'feeds')
    os.makedirs(outdir, exist_ok=True)
    outpath = os.path.join(outdir, f'{name}.xml')
    fg.rss_file(outpath)
    print(f'Wrote {outpath} ({count} entries)')


def main():
    sites = load_sites()
    print(f'Loaded {len(sites)} site configurations from {SITES_JSON}')
    for cfg in sites:
        name = cfg.get('name')
        url = cfg.get('url')
        print(f'--- Processing {name} ({url}) ---')
        html = None
        # prefer rendered file if exists
        rf = cfg.get('render_file')
        if rf:
            # ensure relative paths start with scripts/ if user provided that way
            if not os.path.isabs(rf) and not rf.startswith('scripts'):
                rf = os.path.join('scripts', rf)
            if os.path.exists(rf):
                try:
                    html = open(rf, 'r', encoding='utf-8').read()
                    print(f'Using rendered file: {rf} for {name}')
                except Exception as e:
                    print('Failed reading rendered file:', e)
                    html = None
            else:
                print(f'No rendered file found at {rf} for {name}')
        if html is None:
            # fallback to requests
            try:
                print(f'Fetching {url} via requests...')
                html = fetch_html(url)
            except Exception as e:
                print(f'Request error for {url}: {e}')
                html = ''

        items = []
        if html:
            try:
                items = extract_items_from_html(html, cfg)
                # fallback generic li selector if nothing found
                if not items:
                    soup = BeautifulSoup(html, 'html.parser')
                    nodes = soup.select('li')
                    if nodes:
                        print(f'Fallback: found {len(nodes)} nodes with selector \'li\'')
                        for n in nodes[:200]:
                            title = n.get_text(" ", strip=True)[:200]
                            link = ''
                            a = n.find('a')
                            if a and a.has_attr('href'):
                                link = urljoin(cfg.get('url', ''), a.get('href'))
                            items.append({'title': title, 'link': link, 'description': '', 'date': '', 'full_text': title})
            except Exception as e:
                print('Error parsing HTML:', e)
                items = []
        else:
            items = []

        print(f'Found {len(items)} items for {name} (raw)')

        # apply filters
        matched = []
        kw = cfg.get('filters', {}).get('keywords', [])
        print(f'Applying {len(kw)} keyword filters for {name}: {kw}')
        for it in items:
            keep, reason = matches_filters_debug(it, cfg)
            if keep:
                it['matched_reason'] = reason
                matched.append(it)

        # if nothing matched but items exist, fallback to all items (so you can debug selectors)
        print(f'{len(matched)} items matched filters for {name}')
        if not matched and items:
            print(f'No items matched filters for {name} — falling back to all {len(items)} items')
            matched = items

        # dedupe
        matched = dedupe_items(matched)

        # write feed
        build_feed(name, cfg, matched)


if __name__ == '__main__':
    main()
